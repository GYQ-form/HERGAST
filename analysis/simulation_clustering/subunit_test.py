...
def measure_resources(func):
    def wrapper(*args, **kwargs):
        ...
        if torch.cuda.is_available():
            ...
            allocated = torch.cuda.memory_allocated(device) / (1024 ** 3)  # 转换为 MB
            cached = torch.cuda.memory_reserved(device) / (1024 ** 3)  # 转换为 MB
        else:
            allocated = cached = 0
...
print(f'##############################     set split:{sx},{sy}        ############################')

adata = sc.read_h5ad('data.h5ad') # replace to the simulated data directory

# split = max(round(np.sqrt(adata.shape[0]/int(patch_spots))),1)
true_spots = round(adata.shape[0]/(sx*sy),3)
...
def construct_net(adata):
    ...
    train_HERGAST = HERGAST.Train_HERGAST(adata, batch_data=True, num_batch_x_y=(sx,sy),dim_reduction='PCA',spatial_net_arg={'verbose':False},
...
print(f'Number of patches: {sx*sy} | Number of spots per patch:{true_spots} | ARI = {round(ARI,3)}')
